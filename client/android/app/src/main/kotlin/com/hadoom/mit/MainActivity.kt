...
class MainActivity: FlutterActivity() {
    ...
    override fun configureFlutterEngine(flutterEngine: FlutterEngine) {
        ...
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, CHANNEL).setMethodCallHandler { call, result ->
            when (call.method) {
                ...
                "printLines" -> {
                    val lines = call.argument<List<Map<String, Any?>>>("lines") ?: emptyList()
                    ...
                    executor.execute {
                        ...
                        val printStatus = printLines(lines)
                        runOnUiThread {
                            if (printStatus == SdkResult.SDK_OK) {
                                result.success("Printed Successfully")
                            } else {
    ...
    private fun printLines(lines: List<Map<String, Any?>>): Int {
        var printStatus = mPrinter.printerStatus
        if (printStatus == SdkResult.SDK_PRN_STATUS_PAPEROUT) {
            Log.e("SmartPos", "Printer paper out before batch print")
            return SdkResult.SDK_PRN_STATUS_PAPEROUT
        }

        for (line in lines) {
            val text = line["text"] as? String ?: ""
            val size = (line["size"] as? Number)?.toInt() ?: 24
            val isBold = line["isBold"] as? Boolean ?: false
            val align = (line["align"] as? Number)?.toInt() ?: 0
        ...
        printStatus = mPrinter.setPrintStart()
        Log.d("SmartPos", "Batch print status: $printStatus, lines: ${lines.size}")
        return printStatus
    }
}
